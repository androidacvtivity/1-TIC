...
(
 ...
 CD.CUATM||'~'||CD.NR_ROW  AS NR_ROW,  
  ...
  SUM (CASE WHEN D.RIND  IN ('541') AND D.CAPITOL IN (1185)  AND C.FULL_CODE LIKE '%'||CD.CUATM||';%'  THEN  D.COL1 ELSE 0   END) / 100,1) AS COL2,
  
  ROUND((SUM (CASE WHEN D.RIND  IN ('542') AND D.CAPITOL IN (1185)  AND C.FULL_CODE LIKE '%'||CD.CUATM||';%'  THEN  D.COL1 ELSE 0   END) / 1000)  *
  SUM (CASE WHEN D.RIND  IN ('532') AND D.CAPITOL IN (1185)  AND C.FULL_CODE LIKE '%'||CD.CUATM||';%'  THEN  D.COL1 ELSE 0   END) / 100,1) AS COL3,
  
   CIS2.NVAL(COUNT (DISTINCT(CASE WHEN D.RIND  IN ('560') AND D.CAPITOL IN (1185) AND C.FULL_CODE LIKE '%'||CD.CUATM||';%' AND  CIS2.NVAL(D.COL1)=1 THEN D.CUIIO END))) AS COL4,
